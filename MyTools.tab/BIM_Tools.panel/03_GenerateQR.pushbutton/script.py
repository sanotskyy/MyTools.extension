...
def get_string_param_value(el, param_name):
    ...
    if p and int(p.StorageType) == int(StorageType.String):
        v = p.AsString()
        if v and v.strip():
            return v.strip()
    try:
...
def set_images_for_family(family, png_path, replace_existing, param_name=None):
    ...
    try:
        ...
        if not img_fp:
            ...
            fam_doc.Close(False)
            return u'error: параметр "{}" не знайдено. Доступні: {}'.format(param_name, available)
...
class QRForm(Form):
    def __init__(self, sched_name, url_params, name_params, image_params):
...
def run():
    ...
    frm = QRForm(title, url_params, name_params, image_params)
    frm.ShowDialog()
