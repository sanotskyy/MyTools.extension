...
from Autodesk.Revit.DB import (
    ...
    AssemblyInstance, Element, ViewSchedule,
)
...
def _eid_key(element_id):
    """Повертає ціле число для ElementId незалежно від версії Revit API:
    старіші версії мають .IntegerValue, новіші (2024+) — тільки .Value."""
    try:
...
def _schedule_filter_value_as_string(el, param_name):
    p = None
    try:
...
def _element_passes_schedule_filters(el, definition, field_by_id):
    try:
    ...
    for i in range(filter_count):
        try:
            filt = definition.GetFilter(i)
        except Exception:
            continue
        field = field_by_id.get(filt.FieldId)
        if not field:
            continue
        try:
            ...
            continue
        cur = _schedule_filter_value_as_string(el, param_name) or u""
        try:
            filter_val = filt.GetStringValue()
        except Exception:
            filter_val = None
        fv = filter_val or u""
        ft_name = str(filt.FilterType)
        if ft_name == u"Equal":
            ok = (cur == fv)
        elif ft_name == u"NotEqual":
            ok = (cur != fv)
        elif ft_name == u"Contains":
            ok = (fv in cur)
        elif ft_name == u"NotContains":
            ok = (fv not in cur)
        elif ft_name == u"BeginsWith":
            ok = cur.startswith(fv)
        elif ft_name == u"EndsWith":
            ok = cur.endswith(fv)
        else:
            # Тип фільтра, який ми не обробляємо (напр. числові
            # порівняння) — НЕ відкидаємо елемент навмання, щоб не
            # виключити щось помилково.
            ok = True
        if not ok:
            return False
    return True
...
def build_schedule_count_map():
    u"""Повертає (has_schedule, {element_id_int: 'текст числа'}, diag).

    "Количество" у цій специфікації — вбудоване службове поле Revit
    (ScheduleFieldType.Count), яке показує, скільки однакових елементів
    консолідовано в один рядок при вимкненому "Itemize instances". Це
    НЕ параметр жодного елемента, тому LookupParameter його ніколи не
    знайде — перевірено напряму через Revit Connector: поле приховане
    (IsHidden=True), а після тимчасового ввімкнення видимості й
    порівняння з незалежним підрахунком підтвердилось: значення "Количество"
    для кожного рядка ТОЧНО дорівнює кількості елементів ОДНОГО ТИПУ
    (family type) серед елементів, що проходять фільтри цієї специфікації.

    Тому підхід тут: узяти категорію й фільтри активної специфікації,
    зібрати елементи цієї категорії, які проходять ці фільтри, згрупувати
    їх за ElementTypeId і порахувати розмір кожної групи — це і є
    "Количество" для кожного елемента групи."""
    result = {}
    diag = []
    ...
    try:
        ...
        diag.append(u"Не вдалося отримати активний вид: {0}".format(ex))
        return False, result, diag

    diag.append(u"Активний вид: '{0}' (тип: {1})".format(
        getattr(view, 'Name', u'?'), type(view).__name__))

    if not isinstance(view, ViewSchedule):
        diag.append(u"Активний вид НЕ є специфікацією (ViewSchedule).")
        return False, result, diag
    ...
    try:
        ...
        diag.append(u"Не вдалося прочитати категорію специфікації: {0}".format(ex))
        return True, result, diag

    try:
        els = list(FilteredElementCollector(doc)
                   .OfCategoryId(cat_id)
                   .WhereElementIsNotElementType())
    except Exception as ex:
        diag.append(u"FilteredElementCollector по категорії кинув виняток: {0}".format(ex))
        return True, result, diag

    diag.append(u"Елементів у категорії специфікації: {0}.".format(len(els)))

    field_by_id = {}
    try:
        for fid in definition.GetFieldOrder():
            field_by_id[fid] = definition.GetField(fid)
    except Exception as ex:
        diag.append(u"Не вдалося прочитати поля специфікації: {0}".format(ex))

    matched = [el for el in els if _element_passes_schedule_filters(el, definition, field_by_id)]
    diag.append(u"Після застосування фільтрів специфікації: {0} елемент(ів).".format(len(matched)))
    ...
    for el in matched:
        try:
            ...
            continue
        by_type.setdefault(_eid_key(tid), []).append(el)

    for tid_key, members in by_type.items():
        count_str = u"{0}".format(len(members))
        for el in members:
            try:
                result[_eid_key(el.Id)] = count_str
            except Exception:
                continue

    diag.append(u"Згруповано за типом: {0} груп(и), зібрано значень для {1} елемент(ів).".format(
        len(by_type), len(result)))
...
_HAS_ACTIVE_SCHEDULE, _SCHEDULE_COUNT_MAP, _SCHEDULE_DIAG = build_schedule_count_map()

if _HAS_ACTIVE_SCHEDULE and not _SCHEDULE_COUNT_MAP:
    # Показуємо діагностику ЛИШЕ якщо активний вид справді специфікація,
    # але зібрати значення не вдалось — тимчасове допоміжне вікно для
    # налагодження цієї нової функції, легко прибрати пізніше.
    try:
        WinForms.MessageBox.Show(
            u"\n".join(_SCHEDULE_DIAG), u"Параметри — діагностика 'Количество'")
    except Exception:
        pass


def get_value(element, param):
    if param.get('source') == 'schedule_count':
        try:
            eid_int = _eid_key(element.Id)
        except Exception:
            return None
        return _SCHEDULE_COUNT_MAP.get(eid_int)
...
# Псевдо-параметр "Количество" з активної специфікації (якщо вона зараз
# відкрита і в ній знайдено відповідну колонку) — див. build_schedule_count_map().
if _HAS_ACTIVE_SCHEDULE and _SCHEDULE_COUNT_MAP:
    schedule_src_params = [{
        'name':         u"Количество",
        'level':        'schedule',
...
def elem_src_label(p):
    if p.get('source') == 'schedule_count':
        return u"{} [специфікація]".format(p['name'])
...
copy_src_params          = elem_src_params + room_src_params + schedule_src_params
